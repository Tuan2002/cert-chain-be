...
export class OrganizationMailQueueService {
  ...
  */
  ...
  /**   * Add a job to send manager added email to the queue
   * @param data - Data for the manager added email job
   * @returns a Promise that resolves when the job is added
   */
  async addSendManagerAddedEmailJob(
    data: SendManagerAddedJob
  ): Promise<void> {
    await this.organizationMailQueue.add(OrganizationMailJobs.SEND_MANAGER_ADDED_EMAIL, data, {
      ...DEFAULT_JOB_OPTIONS,
    });
    this.logger.log(
      `Added job to send manager added email to queue for: ${data.to}`
    );
  }
}